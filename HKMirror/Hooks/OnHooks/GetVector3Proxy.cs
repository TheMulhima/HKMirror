...
public static class OnGetVector3Proxy
{
    internal static class HookHandler
    {
        ...
        internal static void HookInvoke()
        {
            if (!HookedList.Contains("Invoke"))
            {
                ...
                new Hook(ReflectionHelper.GetMethodInfo(typeof(GetVector3Proxy), "Invoke"), Invoke);
            }
        }
        ...
        private static Vector3 Invoke(Func<GetVector3Proxy, string, Vector3, Vector3> orig, GetVector3Proxy self,
        ...
        internal static void HookEndInvoke()
        {
            if (!HookedList.Contains("EndInvoke"))
            {
                ...
                new Hook(ReflectionHelper.GetMethodInfo(typeof(GetVector3Proxy), "EndInvoke"), EndInvoke);
            }
        }
        ...
        private static Vector3 EndInvoke(Func<GetVector3Proxy, IAsyncResult, Vector3> orig, GetVector3Proxy self,
            IAsyncResult result)
        {
    ...
    {
        public delegate Vector3 EndInvoke_AfterArgs(Params_EndInvoke args, Vector3 ret);

        public delegate void EndInvoke_BeforeArgs(Params_EndInvoke args);

        public delegate Vector3 EndInvoke_WithArgs(Func<GetVector3Proxy, IAsyncResult, Vector3> orig,
            GetVector3Proxy self, IAsyncResult result);

        public delegate Vector3 Invoke_AfterArgs(Params_Invoke args, Vector3 ret);

        public delegate void Invoke_BeforeArgs(Params_Invoke args);

        public delegate Vector3 Invoke_WithArgs(Func<GetVector3Proxy, string, Vector3, Vector3> orig,
            GetVector3Proxy self, string name, Vector3 orig_);

        public sealed class Params_Invoke
        {
            public string name;
            public Vector3 orig_;
            public GetVector3Proxy self;
        }
    ...
    public static class WithOrig
    {
        public static event Delegates.Invoke_WithArgs Invoke
        {
            add => HookEndpointManager.Add<Delegates.Invoke_WithArgs>(
                ReflectionHelper.GetMethodInfo(typeof(GetVector3Proxy), "Invoke"), value);
        ...
        public static event Delegates.EndInvoke_WithArgs EndInvoke
        {
            add => HookEndpointManager.Add<Delegates.EndInvoke_WithArgs>(
                ReflectionHelper.GetMethodInfo(typeof(GetVector3Proxy), "EndInvoke"), value);
