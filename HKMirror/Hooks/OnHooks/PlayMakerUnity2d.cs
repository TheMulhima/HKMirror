...
public static class OnPlayMakerUnity2d
{
    internal static class HookHandler
    {
        ...
        private static void ForwardCollisionToCurrentState(On.PlayMakerUnity2d.orig_ForwardCollisionToCurrentState orig,
            GameObject target, PlayMakerUnity2d.Collision2DType type, Collision2D CollisionInfo)
        {
            Delegates.Params_ForwardCollisionToCurrentState @params = new()
            {
                target = target, type = type, CollisionInfo = CollisionInfo
            };
            if (_beforeForwardCollisionToCurrentState != null)
                foreach (Delegates.ForwardCollisionToCurrentState_BeforeArgs toInvoke in
                         _beforeForwardCollisionToCurrentState.GetInvocationList())
                    try
                    {
            ...
            if (_afterForwardCollisionToCurrentState != null)
                foreach (Delegates.ForwardCollisionToCurrentState_AfterArgs toInvoke in
                         _afterForwardCollisionToCurrentState.GetInvocationList())
                    try
                    {
    ...
    public static class Delegates
    {
        ...
        public delegate void ForwardCollisionToCurrentState_WithArgs(
            Action<GameObject, PlayMakerUnity2d.Collision2DType, Collision2D> orig, GameObject target,
            PlayMakerUnity2d.Collision2DType type, Collision2D CollisionInfo);
        ...
        public delegate void ForwardEventToGameObject_WithArgs(Action<GameObject, string> orig, GameObject target,
            string eventName);

        public delegate RaycastHit2D GetLastRaycastHitInfo_AfterArgs(Params_GetLastRaycastHitInfo args,
            RaycastHit2D ret);

        public delegate void GetLastRaycastHitInfo_BeforeArgs(Params_GetLastRaycastHitInfo args);

        public delegate RaycastHit2D GetLastRaycastHitInfo_WithArgs(Func<Fsm, RaycastHit2D> orig, Fsm fsm);
        ...
        public delegate void RecordLastRaycastHitInfo_WithArgs(Action<Fsm, RaycastHit2D> orig, Fsm fsm,
            RaycastHit2D info);
