using HutongGames.PlayMaker.Ecosystem.Utils;
using MonoMod.RuntimeDetour;
using MonoMod.RuntimeDetour.HookGen;

namespace HKMirror.Hooks.OnHooks;

/// <summary>
///     Contains different types of On Hooks for PlayMakerEvent class.<br />
///     Contains hooks that aren't generated by monomod, and before and after orig hooks for easier hooking.
/// </summary>
public static class OnPlayMakerEvent
{
    internal static class HookHandler
    {
        private static readonly List<string> HookedList = new();

        internal static void HookSendEvent()
        {
            if (!HookedList.Contains("SendEvent"))
            {
                HookedList.Add("SendEvent");
                new Hook(ReflectionHelper.GetMethodInfo(typeof(PlayMakerEvent), "SendEvent"), SendEvent);
            }
        }

        internal static event Delegates.SendEvent_BeforeArgs _beforeSendEvent;
        internal static event Delegates.SendEvent_NormalArgs _afterSendEvent;

        private static bool SendEvent(Func<PlayMakerEvent, PlayMakerFSM, PlayMakerEventTarget, bool> orig,
            PlayMakerEvent self, PlayMakerFSM fromFsm, PlayMakerEventTarget eventTarget)
        {
            Delegates.Params_SendEvent @params = new()
            {
                self = self, fromFsm = fromFsm, eventTarget = eventTarget
            };
            _beforeSendEvent?.Invoke(@params);
            self = @params.self;
            fromFsm = @params.fromFsm;
            eventTarget = @params.eventTarget;
            var retVal = orig(self, fromFsm, eventTarget);
            if (_afterSendEvent != null) retVal = _afterSendEvent.Invoke(@params);
            return retVal;
        }
    }

    /// <summary>
    ///     Contains necessary information to create Hooks. Does not contain any hooks
    /// </summary>
    public static class Delegates
    {
        public delegate void SendEvent_BeforeArgs(Params_SendEvent args);

        public delegate bool SendEvent_NormalArgs(Params_SendEvent args);

        public sealed class Params_SendEvent
        {
            public PlayMakerEventTarget eventTarget;
            public PlayMakerFSM fromFsm;
            public PlayMakerEvent self;
        }
    }

    /// <summary>
    ///     Contains Hooks to that run code before orig(self) is called
    /// </summary>
    public static class BeforeOrig
    {
        public static event Delegates.SendEvent_BeforeArgs SendEvent
        {
            add
            {
                HookHandler._beforeSendEvent += value;
                HookHandler.HookSendEvent();
            }
            remove => HookHandler._beforeSendEvent -= value;
        }
    }

    /// <summary>
    ///     Contains Hooks to that run code after orig(self) is called
    /// </summary>
    public static class AfterOrig
    {
        public static event Delegates.SendEvent_NormalArgs SendEvent
        {
            add
            {
                HookHandler._afterSendEvent += value;
                HookHandler.HookSendEvent();
            }
            remove => HookHandler._afterSendEvent -= value;
        }
    }

    /// <summary>
    ///     Contains all On Hooks, even those that aren't in the On namespace such as API generated functions and property
    ///     getters/setters
    /// </summary>
    public static class WithOrig
    {
        public static event Delegates.SendEvent_NormalArgs SendEvent
        {
            add => HookEndpointManager.Add<Delegates.SendEvent_NormalArgs>(
                ReflectionHelper.GetMethodInfo(typeof(PlayMakerEvent), "SendEvent"), value);
            remove => HookEndpointManager.Remove<Delegates.SendEvent_NormalArgs>(
                ReflectionHelper.GetMethodInfo(typeof(PlayMakerEvent), "SendEvent"), value);
        }
    }
}