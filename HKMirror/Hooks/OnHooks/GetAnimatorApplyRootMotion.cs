using MonoMod.RuntimeDetour;
using MonoMod.RuntimeDetour.HookGen;

namespace HKMirror.Hooks.OnHooks;

/// <summary>
///     Contains different types of On Hooks for GetAnimatorApplyRootMotion class.<br />
///     Contains hooks that aren't generated by monomod, and before and after orig hooks for easier hooking.
/// </summary>
public static class OnGetAnimatorApplyRootMotion
{
    internal static class HookHandler
    {
        private static readonly List<string> HookedList = new();

        internal static void HookReset()
        {
            if (!HookedList.Contains("Reset"))
            {
                HookedList.Add("Reset");
                new Hook(ReflectionHelper.GetMethodInfo(typeof(GetAnimatorApplyRootMotion), "Reset"), Reset);
            }
        }

        internal static event Delegates.Reset_BeforeArgs _beforeReset;
        internal static event Delegates.Reset_AfterArgs _afterReset;

        private static void Reset(Action<GetAnimatorApplyRootMotion> orig, GetAnimatorApplyRootMotion self)
        {
            Delegates.Params_Reset @params = new()
            {
                self = self
            };
            if (_beforeReset != null)
                foreach (Delegates.Reset_BeforeArgs toInvoke in _beforeReset.GetInvocationList())
                    try
                    {
                        _beforeReset?.Invoke(@params);
                    }
                    catch (Exception e)
                    {
                        HKMirrorMod.DoLogError(e);
                    }

            self = @params.self;
            orig(self);
            if (_afterReset != null)
                foreach (Delegates.Reset_AfterArgs toInvoke in _afterReset.GetInvocationList())
                    try
                    {
                        _afterReset.Invoke(@params);
                    }
                    catch (Exception e)
                    {
                        HKMirrorMod.DoLogError(e);
                    }
        }

        internal static void HookOnEnter()
        {
            if (!HookedList.Contains("OnEnter"))
            {
                HookedList.Add("OnEnter");
                new Hook(ReflectionHelper.GetMethodInfo(typeof(GetAnimatorApplyRootMotion), "OnEnter"), OnEnter);
            }
        }

        internal static event Delegates.OnEnter_BeforeArgs _beforeOnEnter;
        internal static event Delegates.OnEnter_AfterArgs _afterOnEnter;

        private static void OnEnter(Action<GetAnimatorApplyRootMotion> orig, GetAnimatorApplyRootMotion self)
        {
            Delegates.Params_OnEnter @params = new()
            {
                self = self
            };
            if (_beforeOnEnter != null)
                foreach (Delegates.OnEnter_BeforeArgs toInvoke in _beforeOnEnter.GetInvocationList())
                    try
                    {
                        _beforeOnEnter?.Invoke(@params);
                    }
                    catch (Exception e)
                    {
                        HKMirrorMod.DoLogError(e);
                    }

            self = @params.self;
            orig(self);
            if (_afterOnEnter != null)
                foreach (Delegates.OnEnter_AfterArgs toInvoke in _afterOnEnter.GetInvocationList())
                    try
                    {
                        _afterOnEnter.Invoke(@params);
                    }
                    catch (Exception e)
                    {
                        HKMirrorMod.DoLogError(e);
                    }
        }

        internal static void HookGetApplyMotionRoot()
        {
            if (!HookedList.Contains("GetApplyMotionRoot"))
            {
                HookedList.Add("GetApplyMotionRoot");
                new Hook(ReflectionHelper.GetMethodInfo(typeof(GetAnimatorApplyRootMotion), "GetApplyMotionRoot"),
                    GetApplyMotionRoot);
            }
        }

        internal static event Delegates.GetApplyMotionRoot_BeforeArgs _beforeGetApplyMotionRoot;
        internal static event Delegates.GetApplyMotionRoot_AfterArgs _afterGetApplyMotionRoot;

        private static void GetApplyMotionRoot(Action<GetAnimatorApplyRootMotion> orig, GetAnimatorApplyRootMotion self)
        {
            Delegates.Params_GetApplyMotionRoot @params = new()
            {
                self = self
            };
            if (_beforeGetApplyMotionRoot != null)
                foreach (Delegates.GetApplyMotionRoot_BeforeArgs toInvoke in
                         _beforeGetApplyMotionRoot.GetInvocationList())
                    try
                    {
                        _beforeGetApplyMotionRoot?.Invoke(@params);
                    }
                    catch (Exception e)
                    {
                        HKMirrorMod.DoLogError(e);
                    }

            self = @params.self;
            orig(self);
            if (_afterGetApplyMotionRoot != null)
                foreach (Delegates.GetApplyMotionRoot_AfterArgs toInvoke in
                         _afterGetApplyMotionRoot.GetInvocationList())
                    try
                    {
                        _afterGetApplyMotionRoot.Invoke(@params);
                    }
                    catch (Exception e)
                    {
                        HKMirrorMod.DoLogError(e);
                    }
        }
    }

    /// <summary>
    ///     Contains necessary information to create Hooks. Does not contain any hooks
    /// </summary>
    public static class Delegates
    {
        public delegate void GetApplyMotionRoot_AfterArgs(Params_GetApplyMotionRoot args);

        public delegate void GetApplyMotionRoot_BeforeArgs(Params_GetApplyMotionRoot args);

        public delegate void GetApplyMotionRoot_WithArgs(Action<GetAnimatorApplyRootMotion> orig,
            GetAnimatorApplyRootMotion self);

        public delegate void OnEnter_AfterArgs(Params_OnEnter args);

        public delegate void OnEnter_BeforeArgs(Params_OnEnter args);

        public delegate void OnEnter_WithArgs(Action<GetAnimatorApplyRootMotion> orig, GetAnimatorApplyRootMotion self);

        public delegate void Reset_AfterArgs(Params_Reset args);

        public delegate void Reset_BeforeArgs(Params_Reset args);

        public delegate void Reset_WithArgs(Action<GetAnimatorApplyRootMotion> orig, GetAnimatorApplyRootMotion self);

        public sealed class Params_Reset
        {
            public GetAnimatorApplyRootMotion self;
        }

        public sealed class Params_OnEnter
        {
            public GetAnimatorApplyRootMotion self;
        }

        public sealed class Params_GetApplyMotionRoot
        {
            public GetAnimatorApplyRootMotion self;
        }
    }

    /// <summary>
    ///     Contains Hooks to that run code before orig(self) is called
    /// </summary>
    public static class BeforeOrig
    {
        public static event Delegates.Reset_BeforeArgs Reset
        {
            add
            {
                HookHandler._beforeReset += value;
                HookHandler.HookReset();
            }
            remove => HookHandler._beforeReset -= value;
        }

        public static event Delegates.OnEnter_BeforeArgs OnEnter
        {
            add
            {
                HookHandler._beforeOnEnter += value;
                HookHandler.HookOnEnter();
            }
            remove => HookHandler._beforeOnEnter -= value;
        }

        public static event Delegates.GetApplyMotionRoot_BeforeArgs GetApplyMotionRoot
        {
            add
            {
                HookHandler._beforeGetApplyMotionRoot += value;
                HookHandler.HookGetApplyMotionRoot();
            }
            remove => HookHandler._beforeGetApplyMotionRoot -= value;
        }
    }

    /// <summary>
    ///     Contains Hooks to that run code after orig(self) is called
    /// </summary>
    public static class AfterOrig
    {
        public static event Delegates.Reset_AfterArgs Reset
        {
            add
            {
                HookHandler._afterReset += value;
                HookHandler.HookReset();
            }
            remove => HookHandler._afterReset -= value;
        }

        public static event Delegates.OnEnter_AfterArgs OnEnter
        {
            add
            {
                HookHandler._afterOnEnter += value;
                HookHandler.HookOnEnter();
            }
            remove => HookHandler._afterOnEnter -= value;
        }

        public static event Delegates.GetApplyMotionRoot_AfterArgs GetApplyMotionRoot
        {
            add
            {
                HookHandler._afterGetApplyMotionRoot += value;
                HookHandler.HookGetApplyMotionRoot();
            }
            remove => HookHandler._afterGetApplyMotionRoot -= value;
        }
    }

    /// <summary>
    ///     Contains all On Hooks, even those that aren't in the On namespace such as API generated functions and property
    ///     getters/setters
    /// </summary>
    public static class WithOrig
    {
        public static event Delegates.Reset_WithArgs Reset
        {
            add => HookEndpointManager.Add<Delegates.Reset_WithArgs>(
                ReflectionHelper.GetMethodInfo(typeof(GetAnimatorApplyRootMotion), "Reset"), value);
            remove => HookEndpointManager.Remove<Delegates.Reset_WithArgs>(
                ReflectionHelper.GetMethodInfo(typeof(GetAnimatorApplyRootMotion), "Reset"), value);
        }

        public static event Delegates.OnEnter_WithArgs OnEnter
        {
            add => HookEndpointManager.Add<Delegates.OnEnter_WithArgs>(
                ReflectionHelper.GetMethodInfo(typeof(GetAnimatorApplyRootMotion), "OnEnter"), value);
            remove => HookEndpointManager.Remove<Delegates.OnEnter_WithArgs>(
                ReflectionHelper.GetMethodInfo(typeof(GetAnimatorApplyRootMotion), "OnEnter"), value);
        }

        public static event Delegates.GetApplyMotionRoot_WithArgs GetApplyMotionRoot
        {
            add => HookEndpointManager.Add<Delegates.GetApplyMotionRoot_WithArgs>(
                ReflectionHelper.GetMethodInfo(typeof(GetAnimatorApplyRootMotion), "GetApplyMotionRoot"), value);
            remove => HookEndpointManager.Remove<Delegates.GetApplyMotionRoot_WithArgs>(
                ReflectionHelper.GetMethodInfo(typeof(GetAnimatorApplyRootMotion), "GetApplyMotionRoot"), value);
        }
    }
}